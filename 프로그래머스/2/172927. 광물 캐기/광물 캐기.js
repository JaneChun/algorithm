function solution(picks, minerals) {
    ...
    fiveMinerals = fiveMinerals.map((minerals) => { // minerals = [ 'diamond', 'diamond', 'diamond', 'iron', 'iron' ]
        return minerals.reduce((acc, v) => { // v = 'diamond'
            if (v === 'diamond') acc[0]++
    ...
    sortedFiveMinerals.forEach((minerals) => { // minerals = [3, 2, 0]
        for (let pickIdx = 0; pickIdx < picks.length; pickIdx++) { // 다이아 : 0, 철 : 1, 돌 : 2
            const pick = picks[pickIdx] // 곡괭이 개수
            if (pick === 0) continue // 곡괭이 0개면 스킵
            
            for (let mineralIdx = 0; mineralIdx < minerals.length; mineralIdx++) { // 다이아 : 0, 철 : 1, 돌 : 2
